#!/usr/bin/env node
/**
 * Download ONNX / Transformers weights. Runtime never hits these URLs — only this script does.
 *
 *   node scripts/fetch-models.mjs --all
 *   node scripts/fetch-models.mjs --tool speech-transcribe
 *   node scripts/fetch-models.mjs --catalog /path/models.json --tool image-remove-background
 */
...
function parseArgs(argv) {
  const out = { all: false, tool: null, out: null, catalogs: [], includeLarge: false };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--all') out.all = true;
    else if (a === '--tool') out.tool = argv[++i] ?? null;
    else if (a === '--out') out.out = argv[++i] ?? null;
    else if (a === '--catalog') out.catalogs.push(argv[++i] ?? null);
    else if (a === '--include-large') out.includeLarge = true;
  }
  out.catalogs = out.catalogs.filter(Boolean);
  return out;
}
...
async function defaultCatalogs() {
  const dirs = [
    join(repoRoot, 'packages/tools-image-ai/src/models/models.json'),
    join(repoRoot, 'packages/tools-speech/src/models/models.json'),
  ];
  const found = [];
  for (const p of dirs) {
    try {
...
async function loadCatalogs(paths) {
  const models = [];
  const raws = [];
  for (const path of paths) {
    const json = JSON.parse(await readFile(path, 'utf8'));
    raws.push(json);
    for (const m of json.models ?? []) models.push(m);
  }
  return { models, raws };
}

function selectModels(models, { all, tool }) {
  if (all) return models;
  if (tool) return models.filter((m) => m.tools?.includes(tool) || m.id === tool);
...
const catalogPaths = args.catalogs.length ? args.catalogs : await defaultCatalogs();
if (!catalogPaths.length) throw new Error('Keine models.json gefunden.');
const { models: all, raws } = await loadCatalogs(catalogPaths);
const models = selectModels(all, args);
const dirs = destDirs(args.out);
await mkdir(dirs[0], { recursive: true });

for (const model of models) {
  if (/AGPL|NON-COMMERCIAL|CC-BY-NC|\bNC\b/i.test(model.license)) {
    console.warn(`skip forbidden license ${model.id}: ${model.license}`);
    continue;
  }
  if (model.webgpuOnly && !args.includeLarge && args.tool !== model.id) {
    console.warn(`skip ${model.id} (WebGPU-only / Größenwarnung). Nutze --tool ${model.id} oder --include-large.`);
    continue;
  }
...
const publicDir = dirs.find((d) => d.includes('apps/web/public/assets/models')) ?? dirs[0];
await mkdir(publicDir, { recursive: true });
await writeFile(join(publicDir, 'models.json'), JSON.stringify(merged, null, 2));
console.log('done');
void readdir;
