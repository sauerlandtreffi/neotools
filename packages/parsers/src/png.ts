...
import { eqAt, fourcc, readU32BE } from './bytes.js';
  ...
  while (i + 12 <= bytes.length) {
    ...
    if (type === 'iTXt') {
      ...
      if (z >= 0) {
        ...
        texts.push({
          key,
          value: new TextDecoder('utf-8', { fatal: false }).decode(data.subarray(z + 1)).slice(0, 200),
        });
        chunk.text = key;
      }
    }
...
/** Decode 8-bit RGB/RGBA/grayscale/indexed PNG (no interlace). */
export function decodePngRgba(bytes: Uint8Array): RgbaImage | undefined {
  ...
  const bpp =
    colorType === 2 ? 3 : colorType === 6 ? 4 : colorType === 0 ? 1 : colorType === 4 ? 2 : colorType === 3 ? 1 : 0;
