...
import { foreignTransferBytes, formatByteCount, formatBytesShort } from '../../lib/network-bytes';
...
export default function TrustBadge({ locale, compact = false }: { locale: Locale; compact?: boolean }) {
  ...
  return (
    <a
      ...
      title={`${formatByteCount(bytes, locale)} — ${locale === 'de' ? 'Bytes an fremde Server, sollte 0 bleiben' : 'bytes to foreign servers, should stay 0'}`}
    >
      <span class="dot" aria-hidden="true" />
      <span class="ws-trust-local">{w(locale, 'local')}</span>
      <span class="ws-trust-net tnum" data-net-bytes>
        {compact ? formatBytesShort(bytes) : `${locale === 'de' ? 'Netz' : 'Net'} ${formatBytesShort(bytes)}`}
      </span>
      {policy && !compact && (
